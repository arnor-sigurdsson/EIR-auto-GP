...
def run_dl_bo_selection(
    ...
    manual_subset_from_gwas: str | Path | None,
) -> Path | None:
...
def _handle_dl_feature_selection_options(
    bim_file: str | Path,
    manual_subset_from_gwas_file: str | Path | None,
...
def gather_eir_snp_attributions(folder_with_runs: Path) -> pd.DataFrame:
    ...
    if df_final is None:
        raise ValueError("No attribution data found in folder")
...
def get_auto_top_n(
    ...
    if fold < len(manual_fractions):
        ...
        for _, row in df_history.iterrows():
            ...
            opt.tell([row["fraction"]], negated_performance)
