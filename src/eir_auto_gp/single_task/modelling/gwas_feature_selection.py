...
def _get_id_split_folder(
    ...
    if ids_folder.exists():
        if fam_file is None:
            raise ValueError("fam_file is required when ids_folder exists")
...
def run_gwas_feature_selection(
    ...
    if not gwas_finished:
        ...
        if filter_config.pre_split_folder is None:
            raise ValueError("pre_split_folder is required for GWAS feature selection")
...
def maybe_make_plink_train_ids_file(pre_split_folder: str) -> None:
    ...
    with open(train_ids_file, "w") as f:
        with open(train_ids) as train_f:
            for line in train_f:
                line = line.strip()
                f.write(f"{line}\t{line}\n")

        with open(val_ids) as val_f:
            for line in val_f:
                line = line.strip()
                f.write(f"{line}\t{line}\n")
