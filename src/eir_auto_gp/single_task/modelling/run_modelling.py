...
@dataclass
class SingleTaskModelInjectionParams:
    fold: int
    output_folder: str
    manual_valid_ids_file: str | None
    genotype_input_source: str
    genotype_subset_snps_file: str | None
    label_file_path: str
...
def get_gwas_manual_subset_file(
    ...
    if "gwas" in feature_selection_tasks:
        if task == "train":
            ...
            return Path(gwas_snps_to_keep_path)
        elif task == "test":
...
def _get_genotype_injections(
    input_source: str,
    genotype_use_snps_file: str | None,
    n_snps: int,
) -> dict[str, Any]:
    base_snp_path = (
...
def _get_output_injections(
    ...
    if output_cat_columns:
        ...
        if all_binary:
            ...
            logger.info(
                "All categorical outputs are binary. Setting loss to BCEWithLogitsLoss."
            )
...
def _get_all_dynamic_injections(
    injection_params: SingleTaskModelInjectionParams, genotype_data_path: str
) -> dict[str, Any]:
    ...
    valid_size = get_dynamic_valid_size(
        num_samples_per_epoch=spe.num_samples_total,
        minimum=batch_size,
    )
