...
def _id_setup_wrapper(
    ...
    pre_split_folder: str | None = None,
    ...
    if pre_split_folder is not None:
        ...
        logger.info("Using pre-split folder: %s", pre_split_folder_path)

        train_path = pre_split_folder_path / "train_ids.txt"
        if not train_path.exists():
            raise FileNotFoundError(f"Could not find train IDs file: {train_path}")

        test_path = pre_split_folder_path / "test_ids.txt"
        ...
        valid_path = pre_split_folder_path / "valid_ids.txt"
    ...
    return list(train_ids), list(valid_ids), list(test_ids)
...
def _split_csv_into_train_and_test(
    ...
    destination: Path,
    only_data: bool = True,
    input_cat_columns: Sequence[str] | None = None,
