...
def get_plink_gwas_command(
    ...
    one_hot_mappings_file: Path | None,
) -> list[str]:
    """
    ...
    """
    ensure_path_exists(path=Path(output_path), is_folder=True)
...
def get_covariate_names(
    ...
    if one_hot_mappings_file:
        with open(one_hot_mappings_file) as f:
    ...
    covariates_to_use = sorted(set(covariates_to_use))
...
def add_plink_format_train_test_files(
    ...
    if Path(ids_folder, "valid_ids.txt").exists():
        ...
        assert train_ids.intersection(valid_ids) == set(), (
...
def gather_all_snps_to_keep(
    ...
    if ld_clump:
        ...
        with open(clumped_snps_path) as f:
            clumped_snps = {line.strip() for line in f if line.strip()}
...
def gather_all_ids(fam_file_path: str | Path, label_file_path: str | Path) -> list[str]:
    df_fam = _read_fam(fam_path=Path(fam_file_path))
    genotype_ids = set(df_fam[1].astype(str))

    labelled_ids = set(
        gather_ids_from_csv_file(file_path=Path(label_file_path), drop_nas=False)
    )
...
def prepare_gwas_label_file(
    ...
    iid_to_fid = dict(zip(df_fam[1], df_fam[0], strict=False))
    ...
    json_out = output_path_obj.parent / "one_hot_mappings.json"
...
def plot_gwas_results(
    gwas_output_path: Path, p_value_line: float | None = None
) -> None:
...
def get_manhattan_plot(df: pd.DataFrame, p_value_line: float | None) -> plt.Figure:
    ...
    ax = manhattanplot(
        ...
        text_kws={
            "fontsize": 10,
            "arrowprops": {"arrowstyle": "-", "color": "k", "alpha": 0.6},
        },
