...
def generate_interaction_snp_graph_figure(
    ...
    if trait is None:
        raise ValueError("Trait name is required for generating interaction graph")
...
def _get_snp_interactions_from_coefficients(
    df_cluster: pd.DataFrame, top_n: int | None = None
) -> dict:
    interactions = {}
...
def _get_snp_pos_map(bim_file_path: Path) -> dict[str, str]:
    ...
    chr_map = dict(zip(df["VAR_ID"], df["BP_COORD"], strict=False))
    return chr_map


def _get_snp_chr_map(bim_file_path: Path) -> dict[str, int]:
    df = read_bim(bim_file_path=str(bim_file_path))
    chr_map = dict(zip(df["VAR_ID"], df["CHR_CODE"], strict=False))
...
def _get_interaction_graph_figure(
    ...
    if manual_order:
        new_pos = {}
        for idx, (_prev_snp, position) in enumerate(pos.items()):
    ...
    cur_colors = sorted({node_color_map[i] for i in graph.nodes})
    for v in cur_colors:
...
def scale_weights(
    ...
    assert all(min_threshold <= weight <= max_threshold for weight in weights), (
