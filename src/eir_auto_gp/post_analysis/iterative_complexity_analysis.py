...
def plot_metric_figure(
    ...
    for p, change in zip(ax.patches, df_parsed[change_col], strict=False):
...
def prepare_dataframe_with_other_terms(
    ...
    for group in df_copy["group"].unique():
        ...
        for _i, row in df_group.iterrows():
...
def get_step_training_iterator(
...
) -> Generator[tuple[ModelReadyObject, str, str]]:
    """
    1. Only tabular
    2. Only genotype
    3. Genotype + tabular
    ...
    # 4
    if run_tabular and running_mro is not None:
    ...
    # 5
    if run_genotype and running_mro is not None:
    ...
    # 8
    if run_tabular and run_genotype and running_mro is not None:
...
def _get_one_hot_iterator(
    ...
    for snp in df_top_non_additive_snps["KEY"]:
        ...
        if not isinstance(maybe_running_mro, ModelReadyObject):
            continue
...
def get_t_term_iterator(
    ...
    def generator():
        ...
        for col in numerical_columns:
            ...
            if not isinstance(running_mro, ModelReadyObject):
                continue
            maybe_running_mro = _merge_operate_and_split(
                model_ready_object=running_mro,
...
def inverse_scale_column(
    ...
    if scaler.mean_ is None or scaler.scale_ is None:
        raise ValueError(f"Scaler not fitted properly for column {col_name}")

    if not hasattr(scaler.mean_, "__getitem__") or not hasattr(
        scaler.scale_, "__getitem__"
    ):
        raise ValueError(f"Scaler attributes are not array-like for column {col_name}")
...
def get_txt_iterator(
    ...
    running_mro: ModelReadyObject,
    top_n: int,
) -> tuple[Generator | None, pd.DataFrame, ModelReadyObject]:
    pao = post_analysis_object
    ...
    def generator():
        nonlocal running_mro
        for _idx, row in txt_candidates.iterrows():
...
def _find_txt_candidates(
    ...
    for col1, col2 in combinations(iterable=tabular_columns, r=2):
        ...
        if not isinstance(modified_mro, ModelReadyObject):
            continue
...
def get_gxt_iterator(
    ...
    def generator():
        ...
        for _idx, row in gxt_candidates.iterrows():
...
def get_gxg_iterator(
    ...
    for _idx, row in top_gxg_candidates.iterrows():
        snp_pair = row["KEY"]
