...
class DataPaths:
    ...
    gwas_attribution_path: Path | None
...
def get_subset_indices_and_names(
    bim_file_path: Path,
    dl_attributions_path: Path | None,
    gwas_attributions_path: Path | None,
    top_n_snps: int,
    ...
    df_dl_attributions: pd.DataFrame | None = None
    df_gwas_attributions: pd.DataFrame | None = None
    ...
    match (df_dl_attributions, df_gwas_attributions):
        case (pd.DataFrame(), None):
            assert df_dl_attributions is not None
        ...
        case (None, pd.DataFrame()):
            assert df_gwas_attributions is not None
            df_top_snps = get_gwas_top_n_snp_list_df(
                df_gwas=df_gwas_attributions,
                top_n_snps=top_n_snps,
            )
        case (pd.DataFrame(), pd.DataFrame()):
            assert df_dl_attributions is not None and df_gwas_attributions is not None
...
def _ensure_all_snps_present(df_bim: pd.DataFrame, top_snps_list: list[str]) -> None:
    assert all(snp in df_bim["VAR_ID"].values for snp in top_snps_list), (
...
def _ensure_order_of_snps_matches(
    ...
    assert sorted_snps_in_subset == top_snps_list, (
...
def _ensure_alignment_of_top_snps_with_genetic_position(
    ...
    snp_to_position = dict(zip(df_bim["VAR_ID"], df_bim.index, strict=False))
...
def validate_file_paths(dl_path: Path | None, gwas_path: Path | None) -> None:
    ...
    def process_model_data(
        ...
        if task_type == "classification":
            ...
            if label_encoder is None:
                raise ValueError("Label encoder is None for classification task")
...
def apply_genotype_imputers(
    df: pd.DataFrame,
    genotype_imputer: SimpleImputer | None,
) -> pd.DataFrame:
    if genotype_imputer:
...
) -> tuple[SimpleImputer | None, SimpleImputer | None]:
...
def apply_tabular_imputers(
    df: pd.DataFrame,
    continuous_imputer: SimpleImputer | None,
...
class FitTransformers:
    ...
    target_label_encoder: LabelEncoder | None
    numerical_columns: list[str]
    categorical_columns: list[str]
...
class SplitModelData:
    ...
    transformers: FitTransformers | None = None
...
def set_up_split_model_data(
    ...
    if train_ids is None or valid_ids is None:
        raise ValueError("Train and validation IDs are required but not found")
